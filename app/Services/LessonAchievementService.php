...
class LessonAchievementService implements AchievementInterface
{
    /**
     * @var Model
     */
    ...
    public function unlock($userId)
    {
        ...
        foreach ($this->achievements as $achievementName => $achievementCondition) {
            if ($user->watched()->count() >= $achievementCondition['number_of_lessons_to_unlock']) {
                if (! $user->achievements()->where('name', $achievementName)->first()) {
                    $user->achievements()->create([
                        'name' => $achievementName,
                        'achievable_type' => 'App\Models\Lesson',
                        'achievable_count' => $achievementCondition['number_of_lessons_to_unlock'],
                    ]);
